...
namespace Freeroam.Missions.MissionCollection
{
	public class Delivery1 : IMission
	{
		...
		{
			deliveryCar = await World.CreateVehicle(VehicleHash.Kuruma2, new Vector3(666.4f, 676.9f, 128.5f), 193.4f);
			...
			Vehicle vehicle1 = await EntityUtil.CreateVehicle(VehicleHash.Sentinel, new Vector3(851.3f, 505.4f, 125.9f), 343f);
			...
			API.TriggerMusicEvent("IE_START_MUSIC");
			Audio.SetAudioFlag("DisableFlightMusic", true);
			...
			else
			{
				...
				if (!enteredCar)
				{
					if (Game.PlayerPed.CurrentVehicle == deliveryCar)
					{
						...
						API.TriggerMusicEvent("IE_DELIVERING_ATTACK");
		...
		public void Stop()
		{
			...
			API.TriggerMusicEvent("IE_END_MUSIC");
			Audio.SetAudioFlag("DisableFlightMusic", false);
			Audio.SetAudioFlag("WantedMusicDisabled", false);
		}
	}
}
